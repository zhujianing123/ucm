	...
	public static function get_quote($quote_id,$full=true,$skip_permissions=false){
        ...
        if(!$quote){
            ...
            if(module_config::c('quote_name_incrementing',0)){
                ...
                $default_quote_name = $default_quote_name.$quote_number;
            }
...
//            $quote['total_amount_discounted'] = 0;
            ...
            foreach($quote_items as $quote_item){
                if($quote_item['amount'] != 0){
                    // we have a custom amount for this quote_item

                    // wenjie: used to calculate the price of single item

                    // end of calculate
                    if($quote_item['billable']){
                        $quote['total_sub_amount'] += $quote_item['amount'];
                        $total_lite += $quote_item['lite'];
                        if($quote_item['taxable']){
                            $quote['total_sub_amount_taxable'] += $quote_item['amount'] + ($total_lite * 8.86);
                            if(module_config::c('tax_calculate_mode',_TAX_CALCULATE_AT_END)==_TAX_CALCULATE_INCREMENTAL){
                                ...
                                //$quote['total_tax'] += round(($quote_item['amount'] * ($quote['total_tax_rate'] / 100)),module_config::c('currency_decimal_places',2));
                                foreach($quote['taxes'] as $quote_tax_id => $quote_tax){
                                    if(!isset($quote['taxes'][$quote_tax_id]['total'])){
                                        $quote['taxes'][$quote_tax_id]['total'] = 0;
                                    }
                                    $quote['taxes'][$quote_tax_id]['total'] += $quote_item['amount'];
                                    $quote['taxes'][$quote_tax_id]['amount'] += round(($quote_item['amount'] * ($quote_tax['percent'] / 100)),module_config::c('currency_decimal_places',2));
                                }
                            }
                        }
                    }else{
                        $quote['total_sub_amount_unbillable'] += $quote_item['amount'];
                    }
                }
            }
	        
            $quote['total_sub_amount'] += $total_lite * 8.86;
            ...
            $quote['total_sub_amount'] += $moudling*$data['Profile size'];
	...
	public static function duplicate_quote($quote_id){
		...
		$quote_data['name'] = $default_quote_name;
